...
convert_small_numbers <- function(x) {
  if_else(x < 10, xfun::numbers_to_words(x), as.character(x))
}
...
SnapshotDate <- lubridate::ymd("20250930")
...
FormattedDate <- format(SnapshotDate, "%d-%b-%Y")} and the report prepared {format(today(), "%d-%b-%Y")}. The imported data contains a dataframe of {nrow(Snapshot |> filter(RegistryCohortName == "RotatorCuff"))} cases returned from the Rotator Cuff cohort, a dataframe of {nrow(Snapshot |> filter(RegistryCohortName == "GlenohumeralInstability"))} cases returned from the Glenohumeral Instability cohort and a dataframe of {nrow(Snapshot |> filter(RegistryCohortName == "ShoulderGeneral"))} cases from the General cohort. An additional dataframe was retrieved of {nrow(STROBEInput)} cases to generate the STROBE flow chart from the PRULO registry. 
...
#| label: Slice-stack-1
#| code-summary: "filter based on strobe flow"
#| warning: false

MasterTable <- Snapshot |> dplyr::filter(
...
ProcedureMaster2 <- left_join(
  # Filter Master to only include rows with SurgicalTreatment == "Surgical"
 MasterTable1 |> filter(SurgicalTreatment == "Surgical") |>
...
#Calculate status
MasterTable3 <- MasterTable2 |> mutate(
  Status = case_when(
...
MasterFail <- MasterTable3 |> dplyr::filter(
...
MasterSubseq <- MasterTable3 |> dplyr::filter(
  CombID %in% MasterFail$CombID
) |> dplyr::select(
  TreatmentID,
  CombID,
  DateTreatment,
...
MasterRevision <- googlesheets4::range_read(
  ss = SheetIDs$AuditResults,
  sheet = "Failures",
  range = "A1:H",
  col_types = "ccDcccDc"
...
MasterTable4 <- left_join(
  MasterTable3,
  MasterRevision |> dplyr::select(
    TreatmentID,
    SubsequentTx
  ),
  by = "TreatmentID"
...
```{r}
MasterCheck <- MasterTable4 |> dplyr::select(
  TreatmentID,
  RegistryCohortName,
  TreatmentStatus,
  TreatmentStatusNotes,
...
# 
# 
IntraopComp <- MasterTable4 |> dplyr::select(
  TreatmentID,
  CombID,
  DateTreatment,
  SurgicalTreatment,
...
ComplicTable1 <- ComplicTable |> dplyr::filter(
  ...
  mutate(
    ...
    TreatmentID %in% MasterTable4$TreatmentID
  )

#Tidy up TreatmentID in both tables
# Remove leading and trailing whitespaces

MasterTable4$TreatmentID <- trimws(MasterTable4$TreatmentID)
...
ComplicTable2 <- left_join(
  ComplicTable1,
  MasterTable4 |> dplyr::select(
  TreatmentID,
  DateStatusChange,
  DateTreatment,
  ICCount:ICIntervention
...
ComplicFailCheck <- ComplicTable2 |> filter(
...
ComplicInfection <- ComplicTable2 |> dplyr::group_by(TreatmentID) |> filter(Infection == "Yes") |> slice_min(DateOccurFix, n = 1) |> ungroup()
ComplicLigament_Tendon <- ComplicTable2 |> group_by(TreatmentID) |> filter(Ligament_Tendon == "Yes") |> slice_min(DateOccurFix, n = 1) |> ungroup()
ComplicStiffness <- ComplicTable2 |> group_by(TreatmentID) |> filter(Stiffness == "Yes") |> slice_min(DateOccurFix, n = 1) |> ungroup()
ComplicLoosening <- ComplicTable2 |> group_by(TreatmentID) |> filter(Loosening == "Yes") |> slice_min(DateOccurFix, n = 1) |> ungroup()
ComplicHardware <- ComplicTable2 |> group_by(TreatmentID) |> filter(Hardware == "Yes" & Loosening == "No") |> slice_min(DateOccurFix, n = 1) |> ungroup()
ComplicInstability <- ComplicTable2 |> group_by(TreatmentID) |> filter(Instability == "Yes") |> slice_min(DateOccurFix, n = 1) |> ungroup()
ComplicNeurological <- ComplicTable2 |> group_by(TreatmentID) |> filter(Neurological == "Yes") |> slice_min(DateOccurFix, n = 1) |> ungroup()
ComplicFracture <- ComplicTable2 |> group_by(TreatmentID) |> filter(Fracture == "Yes") |> slice_min(DateOccurFix, n = 1) |> ungroup()
ComplicThrombosis <- ComplicTable2 |> group_by(TreatmentID) |> filter(Thrombosis == "Yes") |> slice_min(DateOccurFix, n = 1) |> ungroup()
ComplicEffusion <- ComplicTable2 |> group_by(TreatmentID) |> filter(Effusion == "Yes") |> slice_min(DateOccurFix, n = 1) |> ungroup()
ComplicPain <- ComplicTable2 |> group_by(TreatmentID) |> filter(PainIsol == "Yes") |> slice_min(DateOccurFix, n = 1) |> ungroup()
ComplicOther <- ComplicTable2 |> group_by(TreatmentID) |> filter(Other == "Yes") |> slice_min(DateOccurFix, n = 1) |> ungroup()
ComplicReop <- ComplicTable2 |> group_by(TreatmentID) |> filter(`Reoperation Procedure` == "Yes") |> slice_min(DateOccurFix, n = 1) |> ungroup()
...
ComplicDiscrep <- ComplicTable2 |> filter(
  DateCheck == "Check"|
...
DiagnosisData <- MasterTable4  |> mutate(
...
{
  if(nrow(MasterTable4) == 1) {
    epoxy::epoxy("There is {nrow(MasterTable4)} case involving the anchor of interest.")
  } else {
    procedures <- unique(MasterTable4$ProcedureTotal)
  ...
  paste0(result1, "], and [", procedures[length(procedures)])
    ...
    epoxy::epoxy("There are {nrow(MasterTable4)} cases involving the anchor of interest. Surgeries were performed between {format(min(MasterTable4$DateTreatment),'%Y-%b-%d')} and {format(max(MasterTable4$DateTreatment),'%Y-%b-%d')}. The procedures included [{procedures_string}].")
  }
}
...
MasterTable5 <- MasterTable4 |> mutate(
  ...
  ComplicTable2 |> group_by(TreatmentID) |> slice_max(`Reoperation Procedure Date`, n = 1) |> ungroup() |> dplyr::select(
    TreatmentID,
    ReopDelay_Occur,
    ReopDelay_Surgery,
    EventDelay
...
TableRep2 <-
  ...
  tbl_summary(
      ...
      label = list(
        ...
        CuffRepair ~ "Cuff Remplissage"
      ),
      type = list(
        CuffRepair ~ "dichotomous"
      ),
      value = list(
        CuffRepair ~ "Remplissage"
      ),
      statistic = list(
        all_categorical() ~ "{p} ({n})"
        ),
...
ProductUse <- MasterTable5 |> mutate(
  AnchorCount = stringr::str_count(ImplantCodes,"05.10.002")
) |> summarise(
  Median = median(AnchorCount),
  Mode = get_mode(AnchorCount),
  Max = max(AnchorCount)
)
...
The usage of the anchors per procedure ranged from one to {convert_small_numbers(ProductUse$Max)}, with a median of {convert_small_numbers(ProductUse$Median)}. The most frequent number of anchors implanted per procedure was {convert_small_numbers(ProductUse$Mode)}.
...
The mean follow up duration is {round(MasterTable5 |> ungroup() |> summarise(mean = mean(Duration))/52,1)} years, with a standard deviation of {round(MasterTable5 |> ungroup() |> summarise(sd = sd(Duration))/52,2)} years.
...
```{r}
#| label: tbl-intraopcomp
#| tbl-cap: "Intraoperative complications detected for procedures receiving anchor of interest"


# Check if any Count value is greater than zero in filtered data
has_complications <- MasterTable5 |> 
...
Of the cases identified in @tbl-intraopcomp, all were deemed unrelated to the anchor of interest.
...
MasterTableReop <- ComplicTable2 |> filter(
  TreatmentID %in% MasterTable5$TreatmentID,
...
{ifelse(nrow(MasterTableReop) == 0,
        epoxy::epoxy("There are no reoperations to report."),
       with_gs4_quiet(googlesheets4::range_write(
         ss = SheetIDs$AuditReport,
         data = MasterTableReop,
         sheet = "Reoperations1",
         range = "A1:P",
         col_names = TRUE
         )
