...
The mean follow up duration is {round(MasterTable1 |> ungroup() |> summarise(mean = mean(Duration))/52,1)} years, with a standard deviation of {round(MasterTable1 |> ungroup() |> summarise(sd = sd(Duration))/52,2)} years.
...
has_complications <- MasterTable4 |> 
