...
There are {nrow(MasterTable1 |> filter(is.na(TreatmentID)))} cases with missing TreatmentID in the dataset and {nrow(ComplicTable1 |> filter(is.na(TreatmentID)))} missing in the complications table.
...
# {ifelse(nrow(MasterTable1) <= 1,
#         epoxy::epoxy("There is {nrow(MasterTable1)} case(s) involving the anchor of interest and a Bankart repair. There is insufficient sample size to provide a report."),
#         ifelse(dplyr::between(nrow(MasterTable2BR),2,5),
#         epoxy::epoxy("There are {nrow(MasterTable2BR)} cases involving the anchor of interest and a Bankart repair. Surgeries were performed between {format(min(MasterTable2BR$DateTreatment),'%Y-%b-%d')} to {format(max(MasterTable2BR$DateTreatment),'%Y-%b-%d')}. There is insufficient sample size to provide a report."),
#         epoxy::epoxy("There are {nrow(MasterTable2BR)} cases involving the anchor of interest and a Bankart repair. Surgeries were performed between {format(min(MasterTable2BR$DateTreatment),'%Y-%b-%d')} to {format(max(MasterTable2BR$DateTreatment),'%Y-%b-%d')}."
#         )))} 
# 
# {ifelse(nrow(MasterTable2SLAP) == 1,
#         epoxy::epoxy("There is {nrow(MasterTable2SLAP)} case(s) involving the anchor of interest and a SLAP repair. There is insufficient sample size to provide a report."),
#         ifelse(dplyr::between(nrow(MasterTable2SLAP),2,5),
#         epoxy::epoxy("There are {nrow(MasterTable2SLAP)} cases involving the anchor of interest and a SLAP repair. Surgeries were performed between {format(min(MasterTable2SLAP$DateTreatment),'%Y-%b-%d')} to {format(max(MasterTable2SLAP$DateTreatment),'%Y-%b-%d')}. There is insufficient sample size to provide a report."),
#         epoxy::epoxy("There are {nrow(MasterTable2SLAP)} cases involving the anchor of interest and a SLAP repair. Surgeries were performed between {format(min(MasterTable2SLAP$DateTreatment),'%Y-%b-%d')} to {format(max(MasterTable2SLAP$DateTreatment),'%Y-%b-%d')}."
# )))} 
...
```{r}
#| label: Slice-stack-Rep


MasterTable2 <- MasterTable1 |> mutate(
...
The mean follow up duration is {round(MasterTable2 |> ungroup() |> summarise(mean = mean(Duration))/52,1)} years, with a standard deviation of {round(MasterTable2 |> ungroup() |> summarise(sd = sd(Duration))/52,2)} years.
...
has_complications <- MasterTable2 |> 
...
MasterTable2Other <- MasterTable2 |> filter(
  Other == TRUE
) |> dplyr::select(
  TreatmentID,
  DateTreatment,
...
#| tbl-cap: "Summary of PRULO Report Cases - Postoperative Events"

# Now create the summary table
TableRep64 <- MasterTable2 |>
  dplyr::select(
    Infection,
    Ligament_Tendon,
    Effusion,
...
MasterTable2Reop <- ComplicTable3 |> filter(
  TreatmentID %in% MasterTable2$TreatmentID,
...
Rep6PROM1 <- ggplot(data = MasterTable2PROM1, mapping = aes(Timepoint,QDASH)) + 
geom_boxplot()

Rep6PROM2 <- ggplot(data = MasterTable2PROM1, mapping = aes(Timepoint,WOSINorm)) + 
geom_boxplot()
