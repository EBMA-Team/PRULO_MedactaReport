...
STROBEFlow <- STROBEInput |> dplyr::filter(
  ...
  lost_followup = case_when(
    !is.na(induction) & is.na(exclusion2) & TreatmentStatus == "Failed" & (ymd(DateStatusChange) < ymd(SnapshotDate)) ~ "Repair failure",
    !is.na(induction) & is.na(exclusion2) & TreatmentStatus == "No further followup" & (ymd(DateStatusChange) < ymd(SnapshotDate)) ~ "Patient Opt-out",
...
#| tbl-cap: "Summary of PRULO Report (All) Cases - Procedure Survival"
...
PROMsMaster <- googlesheets4::sheet_write(
  data = MasterTable2 |> dplyr::filter(
    Labrum == "None"
  ) |> left_join(
...
MedactaDiscrep <- MasterTable2 |> dplyr::filter(
    Labrum == "None"
  ) |> dplyr::select(
    TreatmentID,
    Labrum
