...
SnapshotDate <- lubridate::ymd("20250831")
...
```{r}
mismatched_rows <- ComplicTable1[!(ComplicTable1$TreatmentID %in% MasterTable$TreatmentID), ]
...
DiagnosisData <- MasterTable  |> mutate(
...

{
  ...
  paste0(result1, "], and [", procedures[length(procedures)])
    ...
    epoxy::epoxy("There are {nrow(MasterTable)} cases involving the anchor of interest. Surgeries were performed between {format(min(MasterTable$DateTreatment),'%Y-%b-%d')} and {format(max(MasterTable$DateTreatment),'%Y-%b-%d')}. The procedures included [{procedures_string}].")
  }
}
